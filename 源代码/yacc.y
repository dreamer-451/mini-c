...
%token STRUCT '(' ')' '[' ']' '{' '}' ';' ',' '.'  //用bison对该文件编译时，带参数-d，生成的exp.tab.h中给这些单词进行编码，可在lex.l中包含parser.tab.h使用这些单词种类码
...
program: ExtDefList    {displayIR($1);displayAST($1,0);}     /*显示语法树,语义分析 */;
         ;
ExtDefList: {$$=NULL;}
...
StructSpecifier: STRUCT OptTag '{' DefList '}' {$$=mknode(STRUCT_DEF,$2,$4,NULL,yylineno);}
...
VarDec:  ID          {$$=mknode(ID,NULL,NULL,NULL,yylineno);strcpy($$->type_id,$1);}   //ID结点，标识符符号串存放结点的type_id
        | VarDec '[' INT ']' {struct node *temp=mknode(INT,NULL,NULL,NULL,yylineno);temp->type_int=$3;$$=mknode(ARRAY_DEC, $1, temp, NULL,yylineno);}//$$=mknode(ARRAY_DEC,$1,$3,NULL,yylineno);
         ;
...
Stmt:   Exp ';'    {$$=mknode(EXP_STMT,$1,NULL,NULL,yylineno);}
      ...
      | FOR '(' ForDec ')' Stmt {$$=mknode(FOR,$3,$5,NULL,yylineno);}
      ;

ForDec: Exp ';' Exp ';' Exp {$$=mknode(FOR_DEC,$1,$3,$5,yylineno);}
       | ';' Exp ';' {$$=mknode(FOR_DEC,NULL,$2,NULL,yylineno);}
       ;
...
Exp:    Exp ASSIGNOP Exp {$$=mknode(ASSIGNOP,$1,$3,NULL,yylineno);strcpy($$->type_id,"ASSIGNOP");}//$$结点type_id空置未用，正好存放运算符
      ...
      | Exp '[' Exp ']' {$$=mknode(EXP_ARRAY,$1,$3,NULL,yylineno);}
      | Exp '.' ID {struct node *temp=mknode(ID,NULL,NULL,NULL,yylineno);strcpy($$->type_id,$3);$$=mknode(EXP_ELE,$1,temp,NULL,yylineno);}//$$=mknode(EXP_ELE,$1,$3,NULL,yylineno);
