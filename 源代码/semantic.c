...
{ // 匹配函数参数
    ...
    for (j = 1; j < num; j++)
    {
        ...
        if (type1 != type2)
        {
            semantic_error(T->pos, "", "The parameter types do not match");
            return 0;
        }
        T = T->ptr[1];
    }
    if (T->ptr[1])
    { //num个参数已经匹配完，还有实参表达式
        semantic_error(T->pos, "", "Too many parameter are called");
        return 0;
    }
    return 1;
}
    ...
    if (T)
    {
        switch (T->kind)
        {
            ...
            case ID: //查符号表，获得符号表中的位置，类型送type
                ...
                if (rtn == -1)
                    semantic_error(T->pos, T->type_id, "Semantic error: Function undefined");
                if (symbolTable.symbols[rtn].flag == 'F')
                    semantic_error(T->pos, T->type_id, "Semantic error: Function name is wrong and function calls cannot be made");
                else
                {
                    opn1.kind = ID;
...
{ 
    ...
    for (i = 0; i < symbolTable.index; i++)
    {
        ...
        printf("%s\t%d\t%s\t%c\t%d\t%d\n",symbolTable.symbols[i].name, symbolTable.symbols[i].level,
               symbolsType,symbolTable.symbols[i].flag, symbolTable.symbols[i].offset,
               symbolTable.symbols[i].paramnum);
    }
    printf("-----------------------------------------------------------\n");
}
...
void print_IR(struct codenode *head)
{
    ...
    do
    {
        ...
        case STAR:
        case DIV:
            printf("(%d) (%c,%s,%s,%s)\n",i,h->op == PLUS ? '+' : h->op == MINUS ? '-' : h->op == STAR ? '*' : '\\',
...
void id_exp(struct node *T)
{
    ...
    rtn = searchSymbolTable(T->type_id);
    if (rtn == -1)
        semantic_error(T->pos, T->type_id, "Semantic error: Variables are referenced without declaring a definition");
    if (symbolTable.symbols[rtn].flag == 'F')
        semantic_error(T->pos, T->type_id, "Semantic error: Is a function name, not a normal variable");
    else
    {
...
void assignop_exp(struct node *T)
{
    ...
    if (T->ptr[0]->kind != ID)
    {
        semantic_error(T->pos, "", "Semantic error: Assignment statement has no left value");
    }
    else
    {
...
void ext_var_list(struct node *T)
{
    ...
    case ID:
        ...
        if (rtn == -1)
            semantic_error(T->pos, T->type_id, "Semantic error: Variables are repeatedly defined");
        else
            T->place = rtn;
        T->num = 1;
        break;
...
void ext_def_list(struct node *T)
{
    ...
    // 语义分析之前先设置偏移地址
    T->ptr[0]->offset = T->offset;
    semantic(T->ptr[0]); //访问外部定义列表中的第一个
    // 之后合并 code
    T->code = T->ptr[0]->code;
    ...
    if (T->ptr[1])
    {
        ...
        semantic(T->ptr[1]); //访问该外部定义列表中的其它外部定义
        T->code = merge(2, T->code, T->ptr[1]->code);
    }
}
...
void func_dec(struct node *T)
{
    ...
    if (rtn == -1)
    {
        semantic_error(T->pos, T->type_id, "Semantic error: Function name reuse, possibly function duplicate definition");
        return;
    }
    else
        T->place = rtn;
...
void param_dec(struct node *T)
{
    ...
    if (rtn == -1)
        semantic_error(T->ptr[1]->pos, T->ptr[1]->type_id, "Semantic error:The parameter name is repeatedly defined");
    else
        T->ptr[1]->place = rtn;
    T->num = 1;                                //参数个数计算的初始值
...
void comp_stm(struct node *T)
{
    ...
    {
        T->ptr[0]->offset = T->offset;
        semantic(T->ptr[0]); //处理该层的局部变量DEF_LIST
        T->width += T->ptr[0]->width;
        T->code = T->ptr[0]->code;
    }
    if (T->ptr[1])
...
void var_def(struct node *T)
{
    ...
    { //处理所以DEC_LIST结点
        ...
        if (T0->ptr[0]->kind == ID)
        {
            ...
            if (rtn == -1)
                semantic_error(T0->ptr[0]->pos, T0->ptr[0]->type_id, "Variables are repeatedly defined");
            else
                T0->ptr[0]->place = rtn;
            T->width += width;
        }
        else if (T0->ptr[0]->kind == ASSIGNOP)
        {
            ...
            if (rtn == -1)
                semantic_error(T0->ptr[0]->ptr[0]->pos, T0->ptr[0]->ptr[0]->type_id, "Variables are repeatedly defined");
            else
            {
                T0->ptr[0]->place = rtn;
...
void return_dec(struct node *T)
{
    ...
    if (T->ptr[0])
    {
        ...
        {
            semantic_error(T->pos, "Semantic error:Returns an error in the value type", "");
            T->width = 0;
            T->code = NULL;
            return;
        }
